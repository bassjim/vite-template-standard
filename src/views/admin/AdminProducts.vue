...
export default {
  ...
  methods: {
    getProducts (pageNum = 1) { // 參數預設值
      ...
      this.$http.get(`${VITE_URL}/api/${VITE_PATH}/admin/products?page=${pageNum}`)
        .then(res => {
    ...
    saveProduct (product) {
      ...
      if (this.operateType === 'modify') {
        api = `${VITE_URL}/api/${VITE_PATH}/admin/product/${product.id}`
      }
      this.$http[this.operateType === 'create' ? 'post' : 'put'](`${api}`, { data: product })
        .then(res => {
    ...
    deleteProduct () {
      this.isLoading = true
      this.$http.delete(`${VITE_URL}/api/${VITE_PATH}/admin/product/${this.tempProduct.id}`, { data: this.tempProduct })
        .then(res => {
